...
public class Ex01 {
	...
	//		다) 생략      : default, 같은 패키지에서는 접근이 가능(상속관계도 받지 않음)
	...
	//  2) 상수 : 한 번 저장하면 변경할 수 없는 데이터를 저장하는 공간
	//			  모든 글자가 대문자, 단어와 단어사이에 _(언더스코어)를 사용할 수 있다
	//
	// 3. 멤버메소드 : 동작, 기능, 작동하는 것
	// 		- 해당 메소드를 호출(실행)하면 메소드안에 존재하는 내용이 실행된다.
	...
	//							특정 메소드일때 특정메소드에 해당하는 예약어를 사용한다.(static)
	...
	//						  한 클래스안에서 같은 이름의 메소드가 여러개 존재하는 것
	//						  (단, 인자의 갯수와 자료형은 달라야 한다.)
	//						오버로딩을 잘 만들면 사용자는 편하다. 개발자는 만드느라 힘들지만
	...
	//	1) 목적 : 멤버필드(변수와 상수)의 초기값을 지정한다. (객체를 만드는게 목적이 아님)
	...
	//	6) 생성자도 오버로딩이 가능하다. 즉, 하나의 클레스안에 여러개의 생성자가 존재할 수 있다. 
