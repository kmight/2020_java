...
public class Ex03 {
	public static void main(String[] args) {
		// 이클립스에서 API 호출 : 해당클래스에 커서를 놓고 shft + f2
		...
		// float과 double은 0.0이상~ 1.0미만에서 난수 발생
		System.out.println("float형 : " + ran.nextFloat());
		System.out.println("double형 : " + ran.nextDouble());
		
		// 특정범위를 지정하는 방법 :
		// nextInt(범위) : 0 ~ 범위 전까지 정수에서 난수 발생
		System.out.println("범위지정 : " + ran.nextInt(5)); // 0~4까지에서 난수
		
		// 2. (int)(nextDouble() * 범위) : 0 ~ 범위 전까지 정수에서 난수 발생
		System.out.println("범위지정 : " + (int)(ran.nextDouble()*5)); // 0~4까지 에서 난수
		
		// 3. Math 클래스 : 전체 메소드가 static이므로 객체 생성 없이 호출가능
		// 1) random()
